...
InterpolateFloat iAdc11(0, INTERPOLATE_DIVISION);
...
void readAdcParameters()
{
	...
	drate[0]      = detuning(masterFrequency, detune[0]);
	pulseWidth[0] = iAdc4.get() * COUNT_OF_ENTRIES;
	amplitude[0]  = iAdc7.get();

	// OSC2
	detune[1]     = iAdc2.get() * 2.0f - 1.0f;
	drate[1]      = detuning(f2, detune[1]);
	pulseWidth[1] = iAdc5.get() * COUNT_OF_ENTRIES;
	amplitude[1]  = iAdc8.get();

	// OSC3
	detune[2]     = iAdc3.get() * 2.0f - 1.0f;
	drate[2]      = detuning(f3, detune[2]);
	pulseWidth[2] = iAdc6.get() * COUNT_OF_ENTRIES;
